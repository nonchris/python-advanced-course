...
\noindent \tb{Ein Programm Packaging} \quad
	...
	\medskip
	\noindent \tb{HTTP-Header-Manipulation mit \pp{requests}} \quad
		
		\noindent Ziel der Aufgabe ist es, den Umgang mit HTTP-Headern bei Webanfragen zu vertiefen. Es wird im Folgenden eine Aufgabe vorgestellt, bei der du gezielt Header wie den \pp{User-Agent} und den \pp{Referer} setzen musst, um auf eine API zugreifen zu können.
		
		\medskip
		\tb{API-Abfrage mit angepassten Headern}:
		\begin{itemize}
			\item Sende eine GET-Anfrage an eine API unter der URL: \url{https://supersecretstuff.miguelcz.com/validate-request}.
			\item Setze die folgenden HTTP-Header:
			\begin{itemize}
				\item \pp{User-Agent}: Nutze einen legitimen Browser-Agenten wie `Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:130.0) Gecko/20100101 SecretStuffApp/42.0`.
				\item \pp{Referer}: Setze den Wert auf `https://supersecretstuff.miguelcz.com`.
			\end{itemize}
			\item Überprüfe den Statuscode der Antwort. Falls der Code nicht 200 ist, gib eine aussagekräftige Fehlermeldung aus.
			\item Falls der Statuscode 200 ist, parse die JSON-Antwort und gib den Inhalt in der Konsole aus.
			\item[] Mögliches Herangehen:
			\item[] \begin{itemize}
				\item Verwende das \pp{requests}-Modul in Python, um die GET-Anfrage zu senden.
				\item Setze die notwendigen HTTP-Header in einem Dictionary und übergib sie an die Anfrage.
				\item Nutze die Methode \pp{response.json()} zum Parsen der Antwort.
			\end{itemize}
		\end{itemize}
